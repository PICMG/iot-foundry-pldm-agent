...
def discover_devices() -> List[dict]:
    """Return list of candidate serial devices with best-effort USB address."""
    devs = []
    paths = sorted(glob.glob('/dev/ttyUSB*') + glob.glob('/dev/ttyACM*'))
    for p in paths:
        usb = get_usb_address(p)
        devs.append({'path': p, 'usb_addr': usb})
    return devs
...
def main(output):
    ...
    if not devs:
        console.print('[red]No /dev/ttyUSB* or /dev/ttyACM* devices found.[/red]')
        return
    ...
    sel = click.prompt("Select devices by index (comma separated), 'all' to select all, 'none' to cancel", default='all')
    sel = sel.strip()
                ...
                for _ in range(max_pdrs):
                    ...
                    if err:
                        ...
                        if handle == 0:
                            ...
                            break
                    pdrs.append(res)
                    handle = res.get('next_handle', 0)
                    if not handle:
                        break

                # Decode PDRs using the export module's decoder
                decoded_pdrs = []
                for r in pdrs:
                    try:
                        decoded = mod.decode_pdr(r.get('pdr_data', b''))
                    except Exception as e:
                        decoded = {'error': f'decode error: {e}'}
                    decoded_pdrs.append({
                        'handle': r.get('handle'),
                        'next_handle': r.get('next_handle'),
                        'pdr_data': r.get('pdr_data'),
                        'decoded': decoded,
                    })
